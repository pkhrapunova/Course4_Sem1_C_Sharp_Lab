...
namespace Lab1._2
{
    internal class Program
    {
        ...
        // Возвращаем массив (параметр-массив)
        static int[] MinInOddColumns(int[,] matrix)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for (int j = 0; j < n; j += 2) // 0,2,4,... => нечетные по человеческому счёту
                {
                    if (matrix[i, j] < min)
                        min = matrix[i, j];
                }
        ...
        static int MinSumParallelAntiDiagonal(int[,] matrix)
        {
            ...
            // Проверяем все диагонали параллельные побочной (сумма индексов i+j = const)
        ...
        static void InputMatrix(int[,] matrix)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    matrix[i, j] = InputInt($"Введите элемент [{i},{j}]: ");
                }
            }
        }
